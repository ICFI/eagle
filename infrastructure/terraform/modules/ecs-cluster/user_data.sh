...
if [ $? -ne 0 ]; then
    ...
    # Pre-install plugins
    docker run -v /mnt/efs/jenkins:/var/jenkins_home jenkins/jenkins:lts-alpine bash -c "export REF=/var/jenkins_home/plugins/ ; /usr/local/bin/install-plugins.sh amazon-ecr amazon-ecs analysis-collector ant build-timeout credentials-binding email-ext findbugs git github-branch-source gradle greenballs jacoco ldap mailer matrix-auth pam-auth pipeline-aws pipeline-github-lib pipeline-stage-view pmd ssh-slaves subversion timestamper workflow-aggregator ws-cleanup"
else
